...
module ApplicationHelper
  ...
  def action_grid_properties landlord
    action_grid_button '.properties', 'warning', { default: 'Properties', icon: 'iconic-home icon-2x',
       path: private_property_mirent_landlord_properties_path(landlord_id: landlord.id)
    }
  end

  def action_grid_tenants property
    action_grid_button '.tenants', 'info', { default: 'Tenant', icon: 'iconic-user icon-2x',
       path: private_property_mirent_landlord_property_tenants_path(landlord_id: parent.id, property_id: property.id)
    }
  end
  ...
  def action_nav_back
    content_tag('li', action_link('.go_back', default: "Back to #{request.referrer[/\/(\w+)\??[^\/]*$/,1]}",
       icon: 'icon-hand-left', path: request.referrer)) if request.referrer
  end
...
##################################
  def custom object, name, kind = 'view'
    ...
    meta = object.class.columns_hash[name]
...
##################################
  ...
  def collection_for_name name
    ...
    case name
      ...
      when 'payment_type'
        { collection: ["ACB", "Cheque","Direct"] }
      when 'plusminus'
        { collection: ["Plus","Minus"] }
      when 'interesttype'
        { collection: ["Simple","Compound"] }
      when /.*gender$/
        { collection: {Male: "MALE", Female: "FEMALE"} }
      when 'measurementunittype', 'sizeUnits'
        { collection: {'Square meters - m²' => "SQM",'Hectares - ha' => "HECTARES"} }
      when 'deliverymethod'
        { collection: ["SMS","Email","Telephone","Mail"] }
      when 'contact_method'
        { collection: ["Email","SMS","Telephone","Mail", "Fax"] }
      when /.*language.*/
        { collection: {English: 'E', Afrikaans: 'A'} }
      when 'commissionerappointment'
        { collection: { Attorney: "ATTORNEY", Representative: "REPRESENTATIVE"} }
      when 'executiverepresentative'
        { collection: {Director: "DIRECTOR", Representative: "REPRESENTATIVE"} }
      when 'suretytype'
        { collection: { Limited: "LIMITED", Unlimited: "UNLIMITED" } }
      when 'partytype', 'tenant_type'
        { collection: {
      ...
      when 'propertytype'
        { collection: {
            Freehold: "FREEHOLD",
            Leasehold: "LEASEHOLD",
            'Sectional Title' => "SECTIONALTITLE",
            Farm: "FARM",
            'Agricultural Holding' => "AGRICULTURALHOLDING",
        }}
      when 'maritaltatus', 'maritalstatus'
        { collection: {
            Unmarried: "UNMARRIED", Widow: "WIDOW", Widower: "WIDOWER", Divorced: "DIVORCED",
            'In Community of Property' => "ICOP",
            'In Community of Property with Exclusions' => "ICOPWITHEXCLUSION",
            'Out of Community of Property' => "OCOP",
            'Customary Marriage: Before Act' => "CUSTOMARYMARRIAGEBEFOREACT",
            'Customary Marriage: 1 Partner In COP' => "CUSTOMARYMARRIAGE1PARTNERIN",
            'Customary Marriage: 1 Partner Out of COP' => "CUSTOMARYMARRIAGE1PARTNEROUT",
            'Customary Marriage: Multiple' => "CUSTOMARYMARRIAGEMULTIPLE",
            'Customary Marriage' => "CUSTOMARYMARRIAGE",
            'Hindu Rites' => "HINDURITES", 'Muslim Rites' => "MULSIMRITES",
            'Laws of Another Country' => "LAWSOFANOTHERCOUNTRY",
            'Civil Union In COP' => "CIVILUNIONICOP",
            'Civil Union In COP with Exclusions' => "CIVILUNIONICOPWITHEXCLUSION",
            'Civil Union Out of COP' => "CIVILUNIONOCOP",
            'Civil Partnership In COP' => "CIVILPARTNERSHIPICOP",
            'Civil Partnership In COP with Exclusions' => "CIVILPARTNERSHIPICOPWITHEXCLUSION",
            'Civil Partnership Out of COP' => "CIVILPARTNERSHIPOCOP",
        } }
      when 'classificationdescription'
        { collection: [
            "Normal",
            "Development Bond",
            "PIP",
            ...
            "Insolvency",
            "Deceased Estate",
            "Occupation date > 90 Days After Instruction",
            "SARS Delay",
            "Switch",
            "Other – Title Deeds in Deeds Office",
            "Other – Costs Outstanding",
            "Other – Delays Caused by Municipality",
            "Other – Title Deed at Absa",
            "Other – Awaiting Borrowers Certificate",
        ] }
      when 'trusttype'
        { collection: {'Inter Vivos' => "INTERVIVOS", 'Mortis Causa' => "MORTISCAUSA"} }
      when 'methodofidentification'
        { collection: {
            'Identity Document' => "IDENTITYDOCUMENT",
            'Incorrect Identity Document' => "IDENTITYDOCUMENTINCORRECT",
            'South African Identity Document' => "SAIDDOCUMENT",
            'Registration Number' => "REGISTRATIONNO",
        } }
      when 'repaymenttype'
        { collection: {
            'Stop Order' => "StopOrder",
            'Debit Order' => "DebitOrder",
            Cheque: "Cheque",
      ...
      when 'property_type'
        { collection: [
            ...
            'Condo',
            'Penthouse',
            'Fractional Title Unit'].sort
        }
      when 'term_in_months'
        { collection: [1,2,3,6,9,12,18,24,36].map {|t|
            ["#{t} #{'month'.pluralize(t)}", t]
          }
        }
      else
        meta_for_name name
